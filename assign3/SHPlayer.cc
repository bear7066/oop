...
SHPlayer::showCards() const
{
    ...
    for(int line = 0; line < kCardHeight; line++) {     //for each all lines
        ...
        for(int c = 0; c < cardCount; c++) {            //loop cards
            cout << " ";
            memcpy(output, card[c == 0 && !faceup ? 13 : cards[c].getPip() % 13][line], kCardWidth);  //copy line to output, if faceup is false and print first card, print blank card.
